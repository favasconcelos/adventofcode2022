...
async function main() {
  ...
  function processMove(line = "") {
    ...
    const [, numberCratesToMove, fromBucketIndex, toBucketIndex] = match;
    // console.log(buckets);
    // console.log({ cratesToMove, fromBucketIndex, toBucketIndex });
    let createsToMove = [];
    for (let move = 0; move < numberCratesToMove; move++) {
      const crate = buckets[fromBucketIndex].pop();
      createsToMove.push(crate);
    }
    createsToMove.reverse();
    createsToMove.forEach((crate) => buckets[toBucketIndex].push(crate));
  }
